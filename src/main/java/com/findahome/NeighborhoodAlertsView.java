...
public class NeighborhoodAlertsView extends StackPane {
    ...
    public NeighborhoodAlertsView() {
        ...
        toggleList.getChildren().addAll(
                createToggleRow("\ud83d\udd14", "Instant Push Notification", "Alert as soon as it hits the market",
                        true, true),
                createToggleRow("\u2709", "Daily Email Digest", "Summary of all new matches daily", false, true),
                createToggleRow("\ud83d\udcac", "SMS Alert", "Standard carrier rates may apply", false, false));

        section3.getChildren().addAll(s3Title, toggleList);

        content.getChildren().addAll(section1, sep1, section2, sep2, section3);
